...
def build_llm_client_or_none() -> LLMClient | None:
    ...
    if not client.has_api_key:
        logger.warning(
            ...
            "sẽ dừng an toàn, không mutate persistent state."
        )
        return None
...
def main() -> None:
    ...
    logger.info(
        "Stage 1 — chọn tối đa 8 tin; LLM ranking chỉ chạy khi được bật "
        "(2 macro, 2 markets, "
    ...
    if not selected_items:
        notify_operator("Không có tin đủ điều kiện để tạo bản tin hôm nay.", dry_run=dry_run)
        logger.info("Dừng sớm: không cập nhật archive/KB/seen/KV.")
        return
    ...
    if not analysis.success:
        notify_operator(
            "Không tạo được bản tin do Gemini tạm thời không khả dụng. "
            "Tin chưa bị đánh dấu đã đọc; có thể thử /refresh lại sau.",
            dry_run=dry_run,
        )
        logger.error(
            "Dừng sau Stage 2: error_code=%s; không cập nhật archive/KB/seen/KV/trends.",
            analysis.error_code,
        )
        return
    ...
    logger.info(
        "Stage 2.5 — ghi context item tạm lên Cloudflare KV trước khi gửi; "
    ...
    if not kv_write_ok:
        logger.warning(
            "Stage 2.5 — KV write thất bại/bỏ qua -> KHÔNG gắn nút 'Hỏi sâu "
            "thêm' để tránh nút chết (xem secret CLOUDFLARE_* nếu cần fix)."
        )

    logger.info("Stage 3 — deliver (Telegram nếu không DRY_RUN)...")
    ...
    logger.info("Stage 3 — kết quả gửi: %s", "THÀNH CÔNG" if delivered_ok else "THẤT BẠI (xem warning ở trên)")

    if not delivered_ok:
        logger.error("Delivery thất bại: không cập nhật archive/KB/seen/trends.")
        return

    logger.info("Stage 4 — persist sau khi delivery thành công...")
    save_archive(analysis.digest_text, ARCHIVE_DIR)
    if analysis.kb_update:
        kb = merge_kb_update(kb, analysis.kb_update)
        save_kb(kb, KB_PATH)
    else:
        logger.warning("Không có kb_update hợp lệ; giữ nguyên knowledge/kb.json.")
    update_seen_ids(SEEN_PATH, [v.item.id for v in verified_items])
    write_trends_to_kv(archive_dir=ARCHIVE_DIR)
    logger.info("Stage 4 — xong: archive, kb.json, seen.json và trends đã cập nhật.")

    if analysis.json_parse_error:
        logger.warning(
            "Stage 2 digest thành công nhưng kb_update không merge được (%s).",
